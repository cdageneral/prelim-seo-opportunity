...
- **Wayne:** "lets add a date range and project selection. So it can be all projects or select a handful of projects. The date range should be custom along with some quick drop downs like this month, last month, quarter, ytd." A **Scope** bar now sits above the cards: a range dropdown (**All time · This month · Last month · This quarter · Year to date · Custom**), a **project picker** (all, or any subset, including the Unattributed bucket), and a **Reset scope** link. The PDF exports whatever the bar is showing.
- **The panel still opens on All time** (Wayne's call), so nothing about the figures you are used to changes until you ask it to.
- **Where each filter is applied, and why.** The date window goes to the two **spend** routes and is applied in SQL (`created_at >= from AND created_at < to`). The project selection is applied **client-side** through shared folds in `lib/usage/rollupView.ts` that re-sum the grand totals from the surviving per-project lines — so narrowing by project is instant and costs no query, and the totals are an exact roll-up of measured rows rather than a second derivation (Const II.6a).
- **A range is a half-open interval `[from, to)`.** Not a detail: with an inclusive end, a call recorded at 23:59:59.5 on the last day of a month would land in *both* that month and the next, and consecutive periods would not sum to the whole. "Last month" therefore ends exactly where "this month" begins. A custom range still **includes its end day** — the stored bound is the following midnight.
- **Hours Saved and Keywords are NOT date-filtered, and the panel says so.** They are current-state figures — they describe what a project holds today — so there is no honest "hours saved in March". They follow the *project* selection but ignore the *date* range, an amber note appears on the Hours Saved card whenever a range is active, and the scope sentence spells it out. Filtering them would have been easy and wrong (Const I.1/I.5).
- **A dated view excludes manual baselines.** A baseline is an anchor for spend from before the ledger existed, so its `created_at` records when someone typed the number in, not when the money was spent — counting it inside a month it did not occur in would be a fabricated attribution. The scope sentence states the exclusion rather than the totals quietly shrinking.
- **Bad input is never coerced into a different window.** An inverted range, or a date that does not exist (`2026-02-31`), resolves to **all time** and the bar says the range was not applied — it does not swap the ends, and it does not drop the unparseable end and silently apply the other as an open-ended window. That last case was a real bug this release's own checks caught before it shipped.
- **The report says what it covers.** Page one carries the scope statement — headed **"THIS REPORT IS FILTERED"** in amber when it is — and every page eyebrow repeats the window and marks a project filter, because pages get printed and passed around one at a time. A report with no scope supplied prints no statement rather than claiming an unfiltered view it cannot vouch for.
- **System-wide alarms stay system-wide.** The unpriced-source and ledger-write alarms are read from the **unscoped** payload: a real fault is not a per-selection fact, and narrowing to three projects must never make a live data-integrity warning disappear.

**Downstream parity (II.6a/II.6b/II.6c)** — the scope is applied once, in the shared folds, and every surface reads that one view: cards, table, totals and the PDF cannot disagree. No client-facing metric changed, so no Assessment PDF section is owed; the report remains the operator-only artifact v7.482 made it, still outside `lib/pdf/` and still never written to public blob storage.

**Verification** — real `next build` compiled successfully with all five `/api/usage*` routes registered; project `tsc --noEmit` clean under the project's own tsconfig, no `target` override (V.1a). Retained suite **2252 PASS / 33 pre-existing FAIL, zero regression delta**. **94 new checks**: 67 static (every preset boundary asserted against explicit local-midnight instants — including the quarter edge at all twelve months, the January year-rollover, and that a call at 23:59 on a custom range's final day falls *inside* it; subset totals verified against an independently computed sum; the scope sentence proven to carry the not-date-scoped and baseline caveats; the statement proven to reach the rendered report, and proven ABSENT on an unfiltered one) and 27 real-event render assertions in both themes (the preset dropdown refetches and sends the window to the two spend routes but **not** to hours; the picker lists every project and narrowing one re-sums without touching the server; the PDF receives the scoped payload whose grand totals reconcile with its own rows). **10 were verified to FAIL on the pristine v7.482 base.** One v7.482 check (`P2`) whose literal this release deliberately changed was **updated with a dated note and two added assertions, never deleted** (V.6).

## v7.482 — 2026-09-04

### The API Usage dashboard exports itself as a PDF

- **Wayne:** "in the api usage panel, lets create a pdf button that it would generate a pdf report of all the usage and costs." A **PDF** button now sits beside Refresh on `/usage` and downloads the whole dashboard as a landscape Letter report: the headline tiles, consumption by provider, every data-integrity alarm, the full per-project table, the rate card, and the Hours Saved credited-and-withheld appendix.
- **The report READS the screen's numbers; it does not recompute them.** The route is a pure serializer (the v7.378 delivery-package pattern): the button posts the four payloads the panel is already holding — `/api/usage`, `/api/usage/cost`, `/api/usage/hours` and the per-project keyword counts — and the template lays those exact objects out. Re-running the three queries server-side would have produced a second, slightly-later set of numbers and reintroduced exactly the screen-vs-report divergence Art. II.6a exists to end.
- **One shared basis, two surfaces (Const II.7).** The payload shapes, provider labels and every formatter moved out of `UsageRollup.tsx` into new **`lib/usage/rollupView.ts`**, which both the panel and the template import. The panel no longer carries private copies, so a new provider label or a changed rounding rule lands on the screen and in the report in the same edit. The one fold the report performs — the Keywords subtotal — is the panel's own `sumKeywordCounts`, so an unknown count is skipped in both places instead of being summed as zero.
- **The report is INTERNAL, and the code enforces it (Const II.6c).** It carries Hours Saved and the full cost position. So: it lives at `lib/usage/pdfTemplate.ts`, deliberately **not** under `lib/pdf/` — that namespace is one of the three the retained suite forbids from touching internal metrics — the route is `POST /api/usage/pdf`, not under `app/api/reports/`, and unlike the assessment PDF and the delivery package it is **never written to a public Vercel blob URL**: the bytes stream straight back to the operator who asked for them and live nowhere else. Every page is stamped "INTERNAL — iQuanti operations. Not for client distribution."
- **Honest gaps travel with it (Const I.5).** No cost payload prints **no dollar figure anywhere** and says why, rather than a page of `$0.00`. No hours payload omits the appendix entirely. An unreadable keyword count prints a warning mark, a project with no keyword data prints a dash — never a zero. And every alarm on the panel is **reproduced**, not quietly dropped: a printed copy that silently lost "these totals are understated" would read as a clean bill of health.
- **No caps (Const I.6).** Every project, every rate-card row and every activity line is printed. The report paginates; it does not truncate. At current scale that is 14 pages.
- **The wait is honest (Const IV.2/IV.3).** A single server-side Chromium render has no derivable "X of N", so the button reports a changing step label plus elapsed seconds — Collecting → Rendering PDF → Saving — and says what is being rendered. Never a bare spinner.

**Downstream parity (II.6a/II.6b/II.6c)** — this release adds an **operator** export of an existing operator-only panel. It adds no client-facing metric and changes no derivation, so no Assessment PDF section is owed; II.6c governs instead and is enforced, not asserted. Surfaces checked: the Assessment PDF, the delivery package, the PPT-prompt route and `lib/export/*` neither import the new template nor changed — proven by 12 containment checks, each with a negative control. The v7.448 Hours Saved containment still holds with the new files present.

**One thing to fix next, unrelated to this release:** production is currently raising the **unpriced-source alarm** for two Anthropic endpoints (`insights`, `seer`) — their spend is missing from every total (Const I.5b). This release makes that gap more visible, not less: it now prints in the report too. The fix is two entries in `lib/usage/pricing.ts`.

**Verification** — real `next build` **compiled successfully** with `/api/usage/pdf` registered as a dynamic function; project `tsc --noEmit` clean under the project's own tsconfig, no `target` override (Const V.1a). Retained suite **2250 PASS / 33 pre-existing FAIL, zero regression delta** against the pristine v7.481 base. **75 new checks**: 57 source-and-render assertions over the real `buildUsageHTML` (the grand total prints exactly as `fmtUSD` renders it · the keyword subtotal equals the shared fold and the fixture proves the two folds differ · every project appears, none truncated · each alarm is reproduced AND absent in its negative-control state · no cost payload yields no `$` at all · every page carries the INTERNAL stamp and the footer page count matches the pages emitted · the template references no app theme token · a project named `<script>` is escaped) and 18 real-click jsdom render assertions in **both themes** (the button carries the same theme tokens as Refresh, so it is legible wherever Refresh is · the click POSTs exactly the four payloads the panel holds and the posted numbers ARE the loaded numbers · the in-flight label carries elapsed seconds · the download is an in-memory blob named `orbitiq-api-usage-<date>.pdf`, never a public URL · a server failure surfaces the reason and the button recovers). **25 of them were verified to FAIL on the pristine base.** The v7.468 undefined-token guard correctly flagged the new print stylesheet; rather than silencing it, the exemption was made **earned** — both print templates are now re-checked against their own `:root`, and that new check was proven to catch a deliberately injected orphan token (dated note, never deleted — V.6).

## v7.481 — 2026-08-29

### The keyword activities move to Keyword Selection; the list panel keeps only its exports

- **Wayne:** "the keyword activities like clearing the data and adding a keyword manual, etc should be done at the keyword selection panel and not the keyword list panel." **Upload CSV**, **Clear All** and **Add Keyword** are gone from the Keyword Landscape header and now live in **Keyword Selection · Step 1**, next to the client-footprint count they act on. The list panel header keeps its title, its counts, the three exports — and a one-line pointer to where the actions went.
- **The exports stay, deliberately** (Wayne's call). CSV / Excel / Taxonomy CSV export the **active filter and journey scope**, and that scope exists only on the list panel; moving them would have silently changed what a download contains.
- **The upload moved with all of its reporting, not a thinner copy.** The wizard's Step-1 upload previously reported only "saved X of N". It now carries the list panel's full accounting verbatim: the duplicate / blank / failed row breakdown, the SERP-features write diagnosis (sent vs stored), and — the one that matters most — the **v7.451 Position-Type warning** when a Semrush export arrives without the "Position Type" column. Two divergent upload implementations became one (Const II.7, the v7.459 lesson).

- **"Clear All" is now "Start over", on purpose.** Step 2 already had a *Clear all* that only DESELECTS categories; a second control with the same name that deletes the project's data was a mis-click waiting to happen. The destructive one is named **Start over**, sits behind a two-tap confirm, appears only when there is something to wipe, and still does exactly what it did: `POST /keywords/reset` deletes every keyword row and every analysis (the Semrush footprint lives in `analyses.semrush_snapshot`, so a true wipe needs both). No hiding. `api_usage`, competitors and brand vocab are project config and are preserved.
- **Add Keyword** kept its contract too: stored as `source: 'custom'`, with the branded flag **detected** from this project's client + competitor domains and brand terms — never asked of the user (Const I.1).

**Downstream parity (II.6a/II.6b)** — this release relocates existing controls; it adds no panel, no sub-view, no client-facing metric and changes no derivation or stored shape. The endpoints called (`/keywords/batch`, `/keywords`, `/keywords/reset`) and their payloads are byte-for-byte what they were. Surfaces checked: Assessment PDF, Executive Summary, Journey, Content Map, Cluster, Product Insights and the delivery package all read the same stored snapshot before and after. No PDF section is owed.

**Verification** — real `next build` compiled successfully (a route file changed: `app/projects/[id]/page.tsx` rewires `onCleared` to the wizard and hands the list panel `onGoToKeywordSelection`); project tsc clean. Retained suite **2350 PASS / 31 pre-existing FAIL, zero regression delta** against the pristine v7.480 base, with the real-Chromium dual-theme gates running (Const V.5). 30 new checks — 18 real-click render assertions driving the wizard (an upload really POSTs its rows and reports saved-of-file-rows · a file with no Position Type still raises the v7.451 warning while a file with it reports the count and drops the warning · the SERP diagnosis survived · Add keyword POSTs the typed keyword, volume, `source: 'custom'` and a detected branded flag · Start over is two-tap, posts nothing on the first tap or on cancel, posts exactly one reset on confirm and fires `onCleared` · a project with nothing to wipe offers no Start over but still offers the upload) plus 12 static checks (nothing left on the list panel, exports kept, the reset is not named "Clear all", props wired to the right component). **25 of the 30 were verified to FAIL on the pristine base**; the 5 that pass on base are pre-existing invariants this release must not break (the exports, the detected branded flag, the wizard's existing upload). The eight v7.289/v7.290/v7.459 upload checks were retargeted to the upload's new home with dated notes, never deleted (V.6).

## v7.480 — 2026-08-29

### The build workflow now lives in ONE place: the Keyword Selection panel

- **Wayne:** "this should be removed from the keyword list panel as this workflow is part of the keyword selection panel." The v7.241 workflow bar — "Let's build the workflow", the four Step cards, the min-volume presets, the related-terms opt-in and the dry-run review modal — is **gone from the Keyword list panel**. Since v7.476 the Keyword Selection wizard already owns pool building end to end (upload → categories → competitors → expand → pre-product), so two separate UIs were posting to the same `/demand-universe` endpoint with their own independent min-volume and related-terms state. Two UIs over one flow WILL drift (the v7.459 lesson), so the duplicate is removed and the wizard is the single source of truth (Const II.7). The Keyword list panel is now purely the keyword LIST.
- **Nothing was dropped on the floor.** One capability lived ONLY on the removed bar — the per-lane "clear" trash for the product and pre-product demand universes. It moved into the wizard's Step 4 and Step 5 as a two-tap **Clear** control: it appears only when that lane is actually built, the first tap asks ("Delete every product topic?"), and only the confirm issues the genuine `DELETE /demand-universe` for that mode (delete, never hide). The client / competitor summary-card trash buttons are untouched and still live on the Keyword list panel; their failure notice now has its own strip (it previously rendered inside the removed bar and would have gone silent).
- **Stale copy removed with it.** The Keyword Landscape Summary intro said "Build the workflow below…" — pointing at a bar that no longer exists. It now reads "The pool is built in Keyword Selection; this panel is where you drill into it by journey."
- **Source labels renamed with the flow.** The "expanded · Step 3" chip, the `expanded = added by Step 3` legend and the row tooltip no longer name a step number that moved (v7.478 lesson) — they name the step: **Expand footprint**.

**Downstream parity (II.6a/II.6b)** — this release removes UI and relocates one control; it adds no panel, no sub-view and no client-facing metric, and changes no derivation. Surfaces checked: Assessment PDF (reads the stored `_demandUniverse` / pool basis — unchanged), Executive Summary, Journey, Content Map, Cluster, Product Insights and the delivery package all read the same stored snapshot before and after. No PDF section is owed.

**Verification** — real-project tsc clean under the project tsconfig (no `target` override); retained suite **2319 PASS / 31 pre-existing FAIL, zero regression delta** against the pristine v7.479 base with the real-Chromium dual-theme gates running (Const V.5). 18 new checks: 13 real-click render assertions driving the wizard (built lane offers Clear · first tap sends nothing · confirm names the lane · cancel sends nothing and restores the control · confirm sends exactly ONE DELETE with the right mode, per lane · an unbuilt lane offers no control at all) and 5 static checks (bar gone, no deep-build runner and no demand-universe request left in the list panel, wizard keeps the numbered steps and the dry-run review contract). All 18 were verified to FAIL on the pristine base — negative control proven. The three v7.270 header checks and four v7.440 review checks were retargeted to the wizard with dated notes, never deleted (V.6).

## v7.479 — 2026-08-27

### Assessment PDF: renamed to "iQ.Impact's Opportunity Engine"; Insights moved to the front (3 designed pages); Product Breakdown pages; empty local page retired

- **Rebrand (Wayne):** every rendered site that said "OrbitIQ" — cover brand line, page footers, appendix workspace line, endbrand — now names **iQ.Impact's Opportunity Engine**. The v7.404 footer platform credit folds into the product name (footer now reads "iQ.Impact's Opportunity Engine · Provided by iQuanti"); the iQuanti-only accountability sites (cover PROVIDED BY, governance, "An iQuanti product") are unchanged (v7.377 rule intact). Scope is the Assessment PDF; the retired name appears nowhere in its rendered output.
- **Empty local page retired (Wayne: "if there is no local data created dont show an empty page").** The v7.407 "competes locally — not measured yet" placeholder page is removed: a project with local intent but no local scan now omits the local section entirely (the Const I.5 default — omitted, never estimated). Projects WITH a local scan render exactly as before.
- **Insights at the front, no more cutoff (Wayne).** The v7.471 Insights section moves from mid-report to directly after the Executive Summary and is split across three designed pages — the cross-panel story (color-coded pattern cards + ranked "where to strike"), the competitive playbook (per-brand cards: doing well / vulnerable / key stat), and named-vs-cited (the quadrant table). The old single page clipped the playbook table at the fixed page edge; every cap now limits the NUMBER of items (stated on-page) and no item's stored text is ever truncated — content still reads the stored machine-verified blob + the panel's quadrant builder verbatim (II.6a).
- **Product Breakdown pages (Wayne: "bring in the data from the product insights panel — prompts, whos ranking, whos being cited, whos winning").** New pages after the Product Insights overview, two product lines per page: the full page-1 ladder (who ranks, client highlighted, field rank stated), the domains AI answers cite on that line, a "who wins" verdict line, and the actual unbranded probe prompts verbatim with NAMED/ABSENT per answer and the platform asked. The PDF route computes the prompts via the SAME shared probeResultsForUmbrella/buildCategoryToUmbrella the panel's prompt drawer reads (II.7); a product with no probe rows shows no prompts block — not probed, never zero (I.5).

**Verification** — real-project tsc clean; retained suite 2138 PASS (22 new v7.479 checks: rebrand sites, no-old-name grep on rendered output, local omission, insights page order + verbatim-text + caps, breakdown ladder/citations/prompts, route shared-basis reads) with the v7.404 footer checks amended by dated note, zero regression delta vs the v7.478 base; real-Chromium render of the full 13-page fixture measured ZERO page overflow (the cutoff class of bug is now machine-caught).
## v7.478 — 2026-08-27
...
- **The new probe plan.** Phase 2 now probes at TWO levels of the stored taxonomy. Product lines (top 30 procedure umbrella roots by demand — the standing Wayne-approved cap, Const I.6) each carry 5 unbranded prompts plus 1 branded prompt. Every DIRECT sub-category of those lines carries 5 unbranded prompts of its own — no branded prompt at the sub level, sentiment stays at the line (Wayne's per-sub count choice, 2026-08-25). On NYDJ this means Jeans gets jeans-level prompts and By Fit gets fit-level prompts, separately.
- **Prompts speak the node's real ranking terms.** Each unbranded framing is phrased around one of the node's top-5 REAL search terms by volume, read from the stored taxonomy membership (`keywordPaths`) — "What are the best companies or providers for wide leg jeans?" — never an invented phrase (Const I.1/III.1b: membership and hierarchy are read, not fabricated). Fewer terms than framings cycle; a term-less node falls back to its v7.473 contextualized label. The stored result `category` remains the NODE NAME — the v7.472 rollup/drawer matching is untouched by design, and a retained check asserts it at source level.
- **Deadline-partial pool (hardening the volume demands).** The prompt pool now takes a hard deadline and stops STARTING new calls at that point — everything already completed is kept and stored (real, paid-for answers), and the snapshot records `coverage: { planned, completed }`. Before this, an over-budget probe was discarded WHOLE by the v7.346 race (kept as a last-resort backstop). Prompt order is lines by demand, then subs by demand, so a partial run always covers the most important nodes first; the panel states any shortfall plainly ("unmeasured, not zero", Const I.5) and the next analysis re-probes.
- **Every sub-category shows its mapped prompts.** The product-level drawer now groups prompts by node — the line's own prompts first under a PRODUCT LINE header, then each sub-category's under its name, each with its named-in count. And every expanded row in the sub-category drill shows THAT node's own prompts with per-platform mentioned/absent flags; a node that carries no prompts (deeper than a direct sub, or cut by the deadline) shows nothing rather than inheriting the line's (honest gap, I.5).
- **Assessment PDF (Const II.6a/II.6b, same release).** The report's sub-category table now carries each node's own probe figure ("probe: named in X of Y prompts") computed by the SAME shared helper the panel reads (`probeResultsForNode` in lib/productInsights.ts — one basis, Const II.7), absent when a level was not probed. Seer reads the same shared builders and needs no fork. No existing metric changed its basis; the probe simply measures more nodes.
- **Verification:** real-project `tsc --noEmit` clean. Retained suite **2,039 pass / 33 pre-existing env fails / zero unexpected delta**, including **18 new v7.474 checks** — the compiled spec builder at REAL SCALE (30 lines + 150 subs = 934 specs, the size of a big finance tree), line 5+1 vs sub 5+0, term phrasing, term cycling, label fallback, category-stays-node-name, the deadline pool keeping a real completed subset with no holes, per-node subtree filtering with the never-inherit rule, and source assertions across synthesize, the panel, the PDF route and the template. One v7.473 retained check amended with a dated note (the flat single-level plan no longer exists by design — never deleted to pass). Existing stored probes render exactly as scanned; the two-level prompts apply from the next analysis onward.

## v7.473 — 2026-08-25

### Fix: probe prompts speak the product line (Wayne: "the prompts by fit dont make sense as this is in context of the fit of the jeans")

- **The problem.** Probe prompts inserted the leaf category name verbatim, so NYDJ's "By Fit" leaf (under Jeans) produced "What are the best companies or providers for by fit?" — a question no buyer would ask and no LLM can anchor to the product. Visible for the first time because v7.472 un-blanked the prompt drawer.
- **The fix.** `composePromptLabel()` in `lib/apis/llmProbe.ts` — a deterministic, pure-TS label composed from the STORED taxonomy's umbrella root (no model call, no taxonomy change, Const III.1b: the hierarchy is read, never fabricated). Rules, in order: a leaf that IS its umbrella (or has none) stays unchanged; a preposition-leading leaf takes the umbrella in FRONT ("By Fit" → "jeans by fit", "For Bad Credit" → "credit cards for bad credit" — this rule deliberately outranks the shared-stem shortcut); a leaf already naming its line stays unchanged ("Mortgage Calculator" under Mortgages); any other attributive leaf takes the umbrella BEHIND ("Balance Transfer" → "balance transfer credit cards"). Phase 2 resolves each probe category's umbrella root with the same cycle-guarded parent walk `buildCategoryToUmbrella` uses and passes the label alongside the name.
- **The stored category is untouched — by design.** The label feeds PROMPT TEXT only; every stored result still carries `category: cat.name`, so the drawer/rollup matching v7.472 just fixed (and every downstream reader of `profoundSnapshot.results[].category` / `.categories[].category`) is unaffected. Existing stored probes render exactly as scanned (point-in-time data, Const I.1); the new labels apply from the next analysis's probe onward.
- **Downstream trace (Const II.6a):** no metric, denominator, or basis changed — prompt PHRASING only. The PDF, Seer, panel and Exec read stored probe results and stats whose shape is byte-identical.
- **Verification:** real-project `tsc --noEmit` clean. Retained suite **2,021 pass / 33 pre-existing env fails / zero non-v7.473 delta**, including **10 new v7.473 checks**: the compiled `composePromptLabel` on all five rule branches (including the for-bad-credit case that forced the preposition rule to outrank the shared-stem shortcut), and source assertions that prompt text uses the label with the raw name as fallback, every spec still stores `category: cat.name`, synthesize passes a composed label per category, and the umbrella walk keeps its cycle guard.

## v7.472 — 2026-08-25

### Fix: Product Insights probe prompts were blank for every category (Wayne: "on the product insights panel when I do a fresh scan the LLM prompts for the categories still shows up blank")

- **Root cause.** The LLM probe snapshot is STORED in `analyses.profoundSnapshot` (source `llm_probe_v2` — Phase 2 writes it there, and the Exec Summary / AI Answer Engines panels read it there). But every Product Insights reader asked for `analysis.llmProbe` — a field the analysis row has never carried — so `probeResultsForUmbrella` always received `null` and the drawer rendered "No probe prompts for this category" for EVERY category, on every project, since the panel shipped in v7.426. No amount of re-scanning could fix it: the data was present and stored the whole time; the read was dead.
- **The fix: one shared resolver.** `probeFromAnalysis()` in `lib/productInsights.ts` resolves `analysis.llmProbe ?? analysis.profoundSnapshot ?? null`, and all four read sites go through it (Const II.7 — one basis, no drift): the panel's `buildProductRows` input, the panel's prompt drawer, the Assessment PDF's product section, and Seer's product build. Beyond the drawer, this also switches ON the probe-derived per-umbrella AI stats inside `buildProductRows` that had silently aggregated over `null`.
- **Downstream trace (Const II.6a):** the PDF route and `lib/seer/core.ts` carried the SAME dead read and are fixed in this release — both now read the stored probe through the shared resolver; nothing is re-derived. No metric changed its basis; a stored real value previously dropped on the floor now reaches the surfaces that always claimed to show it. Pre-probe analyses resolve to `null` exactly as before (honest gap, Const I.5).
- **Verification:** real-project `tsc --noEmit` clean (project tsconfig, no target override). Retained suite **2,011 pass / 33 pre-existing env fails / zero A-B delta**, including **10 new v7.472 checks**: the compiled resolver + drawer filter on the real analysis-row shape (unbranded-only, rolled to the umbrella through the stored taxonomy), a **negative control proving the pre-fix read returns zero rows on the same fixture**, explicit-llmProbe precedence, null safety, and source-level assertions that none of the three consumer files reads `analysis.llmProbe` directly any more. Live proof on NYDJ (fresh scan 2026-08-25): the drawer lists the real analysis-time prompts per category with per-platform mentioned/absent flags, and the per-line probe chips (Claude 0/35, GPT 0/35 …) now populate. No styling change — newly visible rows use tokens already covered by the retained dual-theme gate.

## v7.471 — 2026-08-21

### New panel: Insights, under Executive Summary (Wayne: "create a new panel on the left under executive summary and call it insights … strong over-arching insights as the data relates to Search ranks, AI visibility, prompts, content, how people are talking about the brand … strengths, weaknesses and competitors")

- **The panel.** A new sub-view under Executive Summary (beside View scope) that tells the cross-panel story the stored data proves: a **category thesis**, 3–6 **detected patterns** (each tagged PATTERN / GOOD NEWS / RISK / OPPORTUNITY), a **competitor playbook** (what each rival has solved, where it is exposed, one key stat), and a ranked **where-to-strike** list.
- **Generated, verified, cached — never live-improvised.** The narrative is produced by the same Claude machinery Seer runs on, against the same all-panels DATA CENSUS and read-only guarded tools — and every number in the output is checked **verbatim** against the tool results of that request by the v7.463 fail-closed verifier. A generation that cannot be verified is **refused and never stored** (the panel says so plainly). What passes is stored on the project (`insights_panel`) with a **"✓ N numbers verified"** badge and re-served until you press Regenerate; generation streams live step labels with elapsed time (Const IV.2). Every call is metered into the api_usage ledger under the project (Const I.5b; model already registered).
- **The Seer core is now shared, not forked.** The grounding machinery (context over `buildKwPool` + the III.1a category guard, census, tools, verifier) moved from the Seer route into `lib/seer/core.ts`, imported by both routes — one basis, no drift (Const II.7; the v7.459 parser-fork lesson). Seer's behaviour is byte-identical; its retained checks were amended to the new home with a dated note, never deleted (Const V.6).
- **Source vs. answer quadrant (deterministic).** X = stored AI-answer citations of each brand's domain, Y = stored % of prompts naming the brand — both read from the AI Answer Engines panel's own persisted Profound aggregate. Brand→domain matching is deterministic squash containment (the v7.432 two-word-brand rule); a brand with no matchable cited domain is **listed as unmeasured, never plotted at zero** (Const I.5). Quadrant labels compare each brand to the plotted field's medians and are stated on-panel as a computed view.
- **Demand-coverage table.** Per product line: journey topics required, client topics covered and % of journey, the field average, and the leader — read from the Product Insights panel's own shared Content-Footprint-vs-Journey builders (Const II.6a: read, never re-derived).
- **Market benchmarks (optional).** External scale data (deposits, market share) is not in any OrbitIQ source, so the scale-vs-visibility table renders only from **user-entered rows that each carry a named source**, displayed verbatim and never computed on (Const I.1). No rows, no table — an honest gap with an in-panel editor.
- **Assessment PDF (Const II.6b).** The panel ships its PDF section in this same release: "Insights — the cross-panel story" renders the STORED verified blob plus the quadrant from the same builder the panel GET uses, states the verified-number count, and is omitted entirely when nothing is generated. Nothing is recomputed for the report (Const II.6a).
- **Downstream trace (Const II.6a/II.6b):** no existing metric changed its basis — this release adds surfaces. The new panel's PDF section ships now (above); the delivery package, PPT prompt and exports were checked and none read the new blob (it is a rollup view; adding it there is a Wayne call for a future release).
- **Verification:** real project `tsc --noEmit` clean and the real `next build` compiled the new route (`/api/projects/[id]/insights-panel`). Retained suite **1,996 pass / 33 pre-existing / zero FAIL delta**, including **106 new v7.471 checks**: the real compiled `buildQuadrant` on fixtures (matching, medians, classification, unmatched-never-zero), the fail-closed route invariants (shared core import, bounded repairs, refusal-before-store, strict schema, access wall, ledger label, column migrations in the list + [id] routes), page wiring, a real `buildAssessmentHTML` drive (section present with the blob, absent without), the real `InsightsSection` rendered in jsdom against full and EMPTY fixtures (every empty state honest, no fabricated zeros), and the dual-theme gate — all 22 tokens the panel uses are declared in both scopes (v7.467 lesson) and the reading pairs measure ≥4.5:1 on the card fill in both themes.
- **In-release hardening (same version — found by driving the LIVE feature end-to-end on Synchrony, not by review):** (1) the page's coming-soon fallback enumerated every known section except the new one, so it rendered beneath the panel — 'insights' added to its exclusion list; (2) a generation that ran out of tool turns errored out — it now forces a final no-tools verified answer from what was gathered (the Seer v7.462 pattern); (3) the prompt forbids rounding outright; (4) verified long-precision floats (7.265774378585086%) are display-rounded by a pure TS formatter AFTER verification — format-at-render, never model math; (5) the generator's number gate moved to a rounding-tolerant variant in lib/seer/core: a number passes only verbatim OR as a 0-3-decimal rounding of a value actually present in that request's tool results — invention still fails closed, and Seer keeps its strict verbatim gate. Five new retained checks cover the variant; two existing checks amended with dated notes (V.6). Live proof: the Synchrony generation stored with **✓ 75 numbers verified**, six patterns, playbook and strike list, legible in both themes.
- **Accuracy over flattery (Wayne, same day: "these insights need to [be] accurate and not inflated for overly positive results"):** the first stored thesis said Synchrony "ranks and cites well in its core products" while the same paragraph showed owned pages at 0.4% of citations and page-1 demand share trailing every field average — every NUMBER was verified, but adjectives aren't numbers. New prompt rule: **qualitative claims are benchmarked claims** — never "ranks/cites well", "strong" or "leading" unless the tools show the client at or above the field average/leader on that SAME measure; below average must be said plainly; a lead on one narrow metric never justifies broad praise; the thesis must agree directionally with its own evidence. Blob length caps also raised (strike 1200, playbook 900) after a run was refused on shape. Regenerated live: the stored thesis now reads "last in AI share of voice at 7.3% while Sofi leads at 26.2%" with praise scoped to the two metrics that earned it — **✓ 71 numbers verified**.

## v7.470 — 2026-08-18

### Keyword panel: a checkbox at every level, so a sub-category can be actioned on its own (Wayne: "on the keyword panel, we should have a check box at each sub-category level as well as the parent level. This way we could delete or action a sub category")

- **Every row in the Category Breakdown now carries a checkbox** — the umbrella, and every sub-category beneath it, at any depth. Before this release the checkbox column was populated for top-level categories only, so the three existing bulk actions (**Move to Content Plan**, **Download Excel**, **Delete (hide)**) could only ever be aimed at a whole category. They now aim at exactly the branch you check.
- **A parent and one of its children can never be counted twice.** A parent's totals are the exact arithmetic rollup of its descendants, so checking both would put the same keyword rows through an action twice — the double count Const I.3 forbids. The action set is therefore the **top-most checked nodes only**: check a parent and every row beneath it renders **checked and locked** ("already included by the selected parent category") rather than offering a second, redundant check, and checking a parent drops any already-checked descendants from the set. Measured on the real component: umbrella (2 own keywords) + two sub-categories (8 and 6) → checking the sub-category alone reads **1 category (1 sub-category) · 8 keywords selected**; then also checking the parent reads **1 category · 16 keywords selected** — not 24, and not 2 categories.
- **A branch with a checked child shows the indeterminate dash.** Collapse is display state (v7.469) and selection is not, so a selection inside a shut branch still reports itself on the parent row instead of disappearing.
- **Hide resolves honestly at every depth.** A `path:` sub-category carries its full stored path key, and the `buildKwPool` hide filter already matches that key as a prefix — so hiding a sub-category hides exactly that subtree and nothing above it, and the keyword-to-category associations stay stored and restorable exactly as before (Const II.8). A **derived topic-split sub-cluster** has no stored category identity of its own; hiding it by its parent's name would take the whole category with it, so it is **not** hidden and the panel says so by name ("… is a derived topic split with no stored category of its own — select its parent category, or remove its keywords with the row trash icon"). The hide confirmation states the count the filter can actually address, not the raw selection. An honest gap, never a wrong action (Const I.1 / I.5).
- **Nothing about the data changed.** No metric, denominator, basis or stored value is touched — this is a selection affordance over the existing exact rollups. The per-row trash icon, the brand ladder, the rank-band columns and the totals are all byte-identical in behaviour.
- **Downstream trace (Const II.6a/II.6b):** no panel metric changed, so no rollup can go stale; the Assessment PDF, the delivery package, the PPT prompt and every export were checked and none of them read selection state — it is ephemeral UI state that never leaves the component. No new client-facing metric ships, so no new PDF section is due.
- **Verification:** real project `tsc --noEmit` clean and the real `next build` compiled. Retained suite **1,890 pass / 33 pre-existing / zero FAIL delta against the pristine v7.469 base**, including **27 new v7.470 checks**. Twenty-one of those drive the **real `KwCategorySection` rendered in jsdom with real `click()` events** — collapsed shows one checkbox, expanded shows three, checking the sub-category alone arms the bar on 8 keywords, the parent goes indeterminate, and checking the parent then locks the child and holds the total at 16. Six more are the dual-theme gate: the checkbox accent now composites on the **sub-row fill** as well as the top-level fill, measured at **4.51:1 dark / 6.51:1 light** (minimum 3:1 for a UI control), and no new CSS token was introduced. The v7.419 check on the per-row checkbox label was **updated in place with a dated note** (the label is now depth-aware) rather than deleted, per Const V.6.
...
- Downstream trace (Const II.6a/II.6b): no metric changed — styling only. Surfaces checked: the Assessment PDF's AUDIENCE SEGMENTS page renders its own print palette on white paper (`--blue` 4.30:1, `--good` 3.27:1, `--violet` 8.33:1 — all at 22px bold, clearing the AA large-text bar) and is unaffected; the delivery package and PPT prompt carry no colour. No new panel or client-facing metric, so no new PDF section is due.
