...
export default function RefreshModal({ clientName, lastAnalyzed, keywordsCount, onClose, onRun, clientThreshold, competitorThreshold, onSaveThresholds }: Props) {
  ...
  const presetBtn = (value: number, active: number, color: string): React.CSSProperties => ({
    ...
    background: value === active ? `${color}1A` : '#131325',
    border: `1px solid ${value === active ? color : '#2A2A4A'}`,
    color: value === active ? color : '#7070A0',
    transition: 'all .15s',
  });
      ...
      <div className="relative w-full max-w-lg animate-fade-in" style={{ background: '#0E0E1C', border: '1px solid #2A2A4A', borderRadius: '12px', padding: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
              ...
              <span style={{ fontSize: '10px', padding: '3px 8px', borderRadius: '20px', fontWeight: 500, background: '#0D2B1D', color: '#4ADE80' }}>cost shown before run</span>
            </div>
        ...
        <div style={{ background: '#0F0F1C', border: '0.5px solid #1E1E38', borderRadius: '8px', padding: '12px', marginBottom: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
            ...
            <span style={{ fontSize: '10px', color: '#505070' }}>keywords below the floor are never fetched — and never billed</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
            <span style={{ fontSize: '10px', fontWeight: 600, color: '#38BDF8', letterSpacing: '.04em', width: '70px', flexShrink: 0 }}>Client</span>
            <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
              {VOL_PRESETS.map(p => (
                <button key={p.value} type="button" onClick={() => setClientThresh(p.value)} style={presetBtn(p.value, clientThresh, '#38BDF8')}>
                  {p.label}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '10px', fontWeight: 600, color: '#F59E0B', letterSpacing: '.04em', width: '70px', flexShrink: 0 }}>Competitors</span>
            <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
              {VOL_PRESETS.map(p => (
                <button key={p.value} type="button" onClick={() => setCompetitorThresh(p.value)} style={presetBtn(p.value, competitorThresh, '#F59E0B')}>
                  {p.label}
                </button>
              ))}
            </div>
          </div>
          <p style={{ fontSize: '10px', color: '#505070', margin: '8px 0 0', lineHeight: 1.5 }}>
            {thresholdsChanged
        ...
        {mode === 'gaps' && (
          <div style={{ background: '#131325', border: '0.5px solid #2A2A4A', borderRadius: '8px', padding: '12px', marginBottom: '18px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '7px' }}>
              <Row icon="ti-refresh" iconBg="#1C1C38" iconColor="#8080C0" text={<><strong style={{ color: '#C0C0E0' }}>Client&apos;s current rankings</strong><span style={{ color: '#8080C0' }}> → full footprint re-pulled; positions updated, new keywords added</span></>} />
              <Row icon="ti-search" iconBg="#1C1C38" iconColor="#8080C0" text={<><strong style={{ color: '#C0C0E0' }}>Competitor keywords not in footprint</strong><span style={{ color: '#8080C0' }}> → full footprints pulled, net-new merged</span></>} />
              <Row icon="ti-check" iconBg="#0D2B1D" iconColor="#4ADE80" text={<><strong style={{ color: '#C0C0E0' }}>Existing SERP &amp; LLM probe data</strong><span style={{ color: '#4ADE80' }}> → reused (this is where the savings are)</span></>} />
